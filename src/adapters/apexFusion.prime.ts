...
// adapters/apexPrime.ts

/**
 * @file adapters/chains/apexPrime.ts
 * @summary Apex Fusion Prime (Layer 1) blockchain adapter using UTxORPC
 * @description Implements ChainAdapter interface for Apex Fusion Prime mainnet
 *              using UTxORPC gRPC protocol for real-time events and queries.
 *              Apex Prime is a Cardano-compatible UTXO chain with Ouroboros PoS.
 *              Supports AP3X token staking, delegation, and transaction monitoring.
 */
...
export class ApexPrimeAdapter implements ChainAdapter {
  name = 'apex-prime';
  displayName = 'Apex Fusion Prime';
  chainType = 'utxo' as const;
  supportedChains = ['apex-prime', 'apex-fusion-prime', 'apex-mainnet'];
  ...
  async init(config: ChainAdapterConfig): Promise<void> {
    this.config = config;
    logger.info(`Initializing Apex Fusion Prime adapter for ${config.name}`);
  ...
  private async resubscribeAll(): Promise<void> {
    logger.info('Resubscribing to Apex Prime events after reconnection');
  ...
  private async processTx(tx: any, slot: number, watchedAddress?: string): Promise<void> {
    try {
      ...
      const hasStakingOps = (tx.certificates && tx.certificates.length > 0) || (tx.withdrawals && tx.withdrawals.length > 0);

      const chainEvent: ChainEvent = {
        eventName: hasStakingOps ? 'StakingTransaction' : 'Transaction',
        section: 'apex-prime',
        method: hasStakingOps ? 'StakingTransaction' : 'Transaction',
        data: {
          hash: txHash,
        ...
        try {
            ...
            for (const utxo of utxos) {
                if (utxo.parsedValued) {
                    const output = utxo.parsedValued as any;  // TxOutput type
                ...
                // Note: If parsedValued is undefined, we simply skip this UTXO.
                // This is safe because the UTxORPC node should always provide parsed data for Cardano-compatible chains.
            }
...
export default function createApexPrimeAdapter(): ChainAdapter {
  return new ApexPrimeAdapter();
}